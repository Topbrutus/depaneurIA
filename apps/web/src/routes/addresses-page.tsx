...
const AddressesPage = ({ customer, onCustomerChange }: AddressesPageProps) => {
  ...
  if (!customer) {
    return (
      <div className="page">
        <div className="card">
          <h2>{t.addresses.noAddress}</h2>
          <p className="muted">{t.addresses.createProfileFirst}</p>
          <Link className="btn btn-primary" to="/signup">
            {t.addresses.createProfile}
          </Link>
        </div>
      </div>
    )
  ...
  const handleSubmit = (formValues: AddressFormValues) => {
    ...
    setInfo(editingId ? t.addresses.addressUpdated : t.addresses.addressAdded)
    resetForm()
  }
  ...
  const handleSetDefault = (addressId: string) => {
    ...
    onCustomerChange(updatedCustomer)
    setInfo(t.addresses.defaultUpdated)
  }
  ...
  const handleDelete = (addressId: string) => {
    ...
    onCustomerChange(updatedCustomer)
    setInfo(t.addresses.addressDeleted)
  ...
  return (
    <div className="page">
      <div className="page-header">
        <div>
          <p className="eyebrow">{t.addresses.title}</p>
          <h1>{t.addresses.manageAddresses}</h1>
          <p className="muted">{t.addresses.addressesDescription}</p>
        </div>
      </div>
      ...
      <div className="grid">
        <div className="card">
          <div className="card-header">
            <h3>{editingId ? t.addresses.editExisting : t.addresses.addNewAddress}</h3>
            {editingId && (
          ...
          <AddressForm
            ...
            errors={errors}
            submitLabel={editingId ? t.addresses.update : t.addresses.add}
          />
        ...
        <div className="card">
          <h3>{t.addresses.myAddresses}</h3>
          {customer.addresses.length === 0 && <p className="muted">{t.addresses.noAddressYet}</p>}
            ...
            {customer.addresses.map((address) => (
              <li key={address.id} className={customer.defaultAddressId === address.id ? 'address-card default' : 'address-card'}>
                <div>
                  ...
                  {address.instructions && <p className="note">{t.addresses.notes} : {address.instructions}</p>}
                </div>
          ...
          {defaultAddress && (
            <div className="pill-stack">
              <span className="pill primary">{t.addresses.default}</span>
              <p className="muted">
